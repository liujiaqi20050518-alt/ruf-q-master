...
#if LV_MEM_CUSTOM == 0 && LV_MEM_SIZE < (12ul * 1024ul)
...
void lv_demo_widgets(void)
{
    ...
    // ========== 关键：为交互控件添加焦点 ==========
    lv_group_t *demo_group = lv_group_create();
    lv_group_set_default(demo_group);
    // lv_group_add_obj(demo_group, tv);
    extern lv_indev_t * indev_keypad;
    lv_indev_set_group(indev_keypad,demo_group);  // 设置默认焦点组
